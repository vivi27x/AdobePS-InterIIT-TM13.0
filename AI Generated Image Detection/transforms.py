...
def add_new_channels(image):
    """Appends the Fourier Transform and Local Binary Pattern channels to the image.

    Args:
        image (torch.Tensor): An image in (C, H, W) format.

    Returns:
        torch.Tensor: A tensor of shape (C+2, H, W) format, with the additional channels added
    """
